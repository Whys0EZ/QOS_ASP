...
namespace QOS.Services
{
    public class UserPermissionService : IUserPermissionService
    {
        ...
        public bool HasPermission(string username, string functionCode)
        {
            ...
            var property = typeof(UserPermission).GetProperty(functionCode, BindingFlags.IgnoreCase | BindingFlags.Public | BindingFlags.Instance);
            if (property == null || property.PropertyType != typeof(bool)) return false;

            return (bool)property.GetValue(user);
        }
    }
}
