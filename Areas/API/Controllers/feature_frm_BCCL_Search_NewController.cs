...
namespace QOS.Areas.API.Controllers
{
    ...
    [ApiController]
    public class feature_frm_BCCL_Search_NewController : ControllerBase
    {
        private readonly IConfiguration _config;
        private readonly string _connectionString;
        private readonly ILogger<feature_frm_BCCL_Search_NewController> _logger;

        public feature_frm_BCCL_Search_NewController(IConfiguration config, ILogger<feature_frm_BCCL_Search_NewController> logger)
        {
            _config = config;
            _logger = logger;
        ...
        [HttpGet]
        public IActionResult feature_frm_BCCL_Search_New(
            string? Code_G, 
            string? Search_txt, 
            string? Unit, 
            string? ALL)
        ...
        private List<BCCLCModel> GetBCCLCData(string factoryID, string dateF, string unit, string searchTxt, string all)
        {
            ...
            using (SqlCommand cmd = new("Json_RP_BCCL_SUM_NEW_2", conn))
            {
