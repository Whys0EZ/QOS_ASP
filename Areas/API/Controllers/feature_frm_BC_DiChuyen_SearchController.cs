...
namespace QOS.Areas.API.Controllers
{
    ...
    [ApiController]
    public class feature_frm_BC_DiChuyen_SearchController : ControllerBase
    {
        private readonly IConfiguration _config;
        private readonly string _connectionString;
        private readonly ILogger<feature_frm_BC_DiChuyen_SearchController> _logger;

        public feature_frm_BC_DiChuyen_SearchController(IConfiguration config, ILogger<feature_frm_BC_DiChuyen_SearchController> logger)
        {
            _config = config;
            _logger = logger;
        ...
        [HttpGet]
        public IActionResult feature_frm_BC_DiChuyen_Search(
            string? Code_G, 
            string? Search_txt, 
            string? Unit, 
            string? Line,
            string? ALL)
        {
            ...
            try
            {
                ...
                // _logger.LogInformation($"SearchInLine - FactoryID: {factoryID}, Date: {dateF}");

                // Query database...
                var InLineList = GetInLineData(factoryID, dateF, Unit ?? "",Line ?? "", Search_txt ?? "", ALL ?? "");

                return Ok(new { BC_DiChuyen_Search = InLineList });
            }
            catch (SqlException ex)
            {
                _logger.LogError(ex, "Database error in SearchInLine");
                return StatusCode(500, new { error = "Database error occurred" });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in SearchInLine");
...
// GET api/feature_frm_BC_DiChuyen_Search?Code_G=f4cf3fe4b67244332eecec055742d449REG263ca1464c3480896c1b78a5eac5a6e971f0e3dad99908345f7439f8ffabdffc4_____DeviceName_____MAC_____LoginID_____2025-10-28&Search_txt=&Unit=2U01&Line=201S01&ALL=YES