...
namespace QOS.Areas.API.Controllers
{
    ...
    [ApiController]
    public class Get_DataFromServerToSQLite_MOController : ControllerBase
    {
        private readonly IConfiguration _config;
        private readonly string _connectionString;
        private readonly ILogger<Get_DataFromServerToSQLite_MOController> _logger;

        public Get_DataFromServerToSQLite_MOController(IConfiguration config, ILogger<Get_DataFromServerToSQLite_MOController> logger)
        {
            _config = config;
            _logger = logger;
        ...
        [HttpGet]
        [HttpGet("[action]")]
        public IActionResult Get_DataFromServerToSQLite_MO(string? Code_G)
        {
            ...
            try
            {
                ...
                object response ;
                _logger.LogInformation(" Get ETS_Data_MO_Info: MO " + MO + "  FactoryID : " + FactoryID );
                if (tmp1 == Functions.MD5Hash(facCode) && tmp3 == Functions.MD5Hash(FactoryID))
                {
                    
                    response = new
                    {
                        ETS_Data_MO_Info = ETS_Data_MO_Info(MO, FactoryID)
                    };
                }
                else {
                    response = new
            ...
            {
                _logger.LogError(ex, "Error in ETS_Data_MO_Info");
                return BadRequest(new { error = ex.Message });
            }
        }
        private List<object> ETS_Data_MO_Info(string MO , string FactoryID)
        {
