...
namespace QOS.Areas.API.Controllers
{
    ...
    [ApiController]
    public class Get_SQLite_2_Server_BC_FQC_UQ_DELController : ControllerBase
    {
        private readonly IConfiguration _config;
        private readonly string _connectionString;
        private readonly IWebHostEnvironment _environment;
        private readonly ILogger<Get_SQLite_2_Server_BC_FQC_UQ_DELController> _logger;

        public Get_SQLite_2_Server_BC_FQC_UQ_DELController(IConfiguration config,IWebHostEnvironment environment, ILogger<Get_SQLite_2_Server_BC_FQC_UQ_DELController> logger)
        {
            _config = config;
        ...
        [HttpPost]
        public IActionResult Get_SQLite_2_Server_BC_FQC_UQ_DEL(
            [FromQuery] string? Code_G,
            [FromQuery] string? ID_Report,
            ...
            [FromBody] DeletePhotoRequest? request)
        {
            ...
            if (string.IsNullOrEmpty(ID_Report))
                return BadRequest(new { KQ = "NG: ID_Report is required" });
            ...
            try
            {
                ...
                var rs = DeleteReportFromDatabase(ID_Report, Audit_Time_V ?? "", loginID);
        ...
        private string DeleteReportFromDatabase(string reportID, string auditTimeV, string loginID)
        {
            try
            {
                ...
                using (SqlCommand cmd = new("Json_FQC_UQ_Delete_Report", conn))
                {
