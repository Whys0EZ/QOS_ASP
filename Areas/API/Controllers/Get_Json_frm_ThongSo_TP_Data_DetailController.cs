...
namespace QOS.Areas.API.Controllers
{
    ...
    [ApiController]
    public class Get_Json_frm_ThongSo_TP_Data_DetailController : ControllerBase
    {
        private readonly IConfiguration _config;
        private readonly string _connectionString;
        private readonly ILogger<Get_Json_frm_ThongSo_TP_Data_DetailController> _logger;

        public Get_Json_frm_ThongSo_TP_Data_DetailController(IConfiguration config, ILogger<Get_Json_frm_ThongSo_TP_Data_DetailController> logger)
        {
            _config = config;
            _logger = logger;
        ...
        [HttpGet]
        public IActionResult Get_Json_frm_ThongSo_TP_Data_Detail(
            string? Code_G, 
            string? WorkDate, 
            string? FactoryID, 
            string? TypeName,
            ...
            string? SizeNo)
        {
            ...
            try
            {
                ...
                // _logger.LogInformation($"ThongSo_TP_Detail - FactoryID: {factoryID}, Date: {dateF}");

                // Query database...
                var ThongSoBTPDetail = ThongSo_TP_Detail(WorkDate, FactoryID, TypeName,WorkStageName, LINE_No, MO, ColorCode, Pattern, TableCode, Size, SizeNo);

                return Ok(new { Json_Get_ThongSo_TP_Detail = ThongSoBTPDetail });
            }
            catch (SqlException ex)
            {
                _logger.LogError(ex, "Database error in ThongSo_TP_Detail");
                return StatusCode(500, new { error = "Database error occurred" });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in ThongSo_TP_Detail");
                return BadRequest(new { error = "An error occurred processing your request" });
            }
        }

        private List<object> ThongSo_TP_Detail(string? WorkDate,string? FactoryID,string? TypeName,string? WorkStageName,string? LINE_No,string? MO,string? ColorCode,string? Pattern,string? TableCode,string? Size,string? SizeNo)
        {
            List<object> list = new();

            using (SqlConnection conn = new(_connectionString))
            using (SqlCommand cmd = new("Json_Get_ThongSo_TP_Detail", conn))
            {
                ...
                while (dr.Read())
                {
                    list.Add(new 
                    {
                        ...
                        PatternCode = Functions.GetStringValue(dr, "PatternCode"),
...
// GET api/Get_Json_frm_ThongSo_TP_Data_Detail?Code_G=f4cf3fe4b67244332eecec055742d449REG263ca1464c3480896c1b78a5eac5a6e971f0e3dad99908345f7439f8ffabdffc4_____DeviceName_____MAC_____LoginID_____2025-10-28&Search_txt=&Unit=2U01&Line=201S01&ALL=YES