...
namespace QOS.Areas.API.Controllers
{
    ...
    [ApiController]
    public class Get_DataFromServerToSQLite_Emp_Info_1VController : ControllerBase
    {
        private readonly IConfiguration _config;
        private readonly string _connectionString;
        private readonly ILogger<Get_DataFromServerToSQLite_Emp_Info_1VController> _logger;

        public Get_DataFromServerToSQLite_Emp_Info_1VController(IConfiguration config, ILogger<Get_DataFromServerToSQLite_Emp_Info_1VController> logger)
        {
            _config = config;
            _logger = logger;
        ...
        [HttpGet]
        public IActionResult Get_DataFromServerToSQLite_Emp_Info_1V(string? Code_G)
        {
            ...
            try
            {
                ...
                if (tmp1 == Functions.MD5Hash(facCode) && tmp3 == Functions.MD5Hash(FactoryID) && CardNo !="")
                {
                    ...
                    response = new
                    {
                        Emp_Infor = Emp_Infor(CardNo, Type_c, FactoryID)
                    };
                }
                else {
                    response = new
        ...
        private List<object> Emp_Infor(string CardNo , string Type_c, string FactoryID)
        {
            ...
            using (SqlCommand cmd = new("Json_Get_Emp_Infor_1V", conn))
            {
