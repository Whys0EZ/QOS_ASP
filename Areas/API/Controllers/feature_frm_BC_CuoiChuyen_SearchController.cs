...
namespace QOS.Areas.API.Controllers
{
    ...
    [ApiController]
    public class feature_frm_BC_CuoiChuyen_SearchController : ControllerBase
    {
        private readonly IConfiguration _config;
        private readonly string _connectionString;
        private readonly ILogger<feature_frm_BC_CuoiChuyen_SearchController> _logger;

        public feature_frm_BC_CuoiChuyen_SearchController(IConfiguration config, ILogger<feature_frm_BC_CuoiChuyen_SearchController> logger)
        {
            _config = config;
            _logger = logger;
        ...
        [HttpGet]
        public IActionResult feature_frm_BC_CuoiChuyen_Search(
            string? Code_G, 
            string? Search_txt, 
            string? Unit, 
            string? Line,
            string? ALL)
        {
            ...
            try
            {
                ...
                // _logger.LogInformation($"SearchEndLine - FactoryID: {factoryID}, Date: {dateF}");

                // Query database...
                var EndLineList = GetEndLineData(factoryID, dateF, Unit ?? "",Line ?? "", Search_txt ?? "", ALL ?? "");

                return Ok(new { BC_DiChuyen_Search = EndLineList });
            }
            catch (SqlException ex)
            {
                _logger.LogError(ex, "Database error in SearchEndLine");
                return StatusCode(500, new { error = "Database error occurred" });
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in SearchEndLine");
                return BadRequest(new { error = "An error occurred processing your request" });
            }
        }

        private List<object> GetEndLineData(string factoryID, string dateF, string unit,string line, string searchTxt, string all)
        {
            List<object> list = new();

            using (SqlConnection conn = new(_connectionString))
            using (SqlCommand cmd = new("Json_RP_BC_CuoiChuyen_SUM", conn))
            {
                ...
                while (dr.Read())
                {
                    list.Add(new 
                    {
                        ...
                        Check_QTY = Functions.GetStringValue(dr, "Check_QTY"),
                        Total_Fault_QTY = Functions.GetStringValue(dr, "Total_Fault_QTY"),
                        Fault_AQL_QTY = Functions.GetStringValue(dr, "Fault_AQL_QTY"),
                        ...
                        Re_Audit_Time = Convert.ToDateTime(dr["Re_Audit_Time"]).ToString("dd-MMM-yyyy HH:mm:ss"),
...
// GET api/feature_frm_BC_CuoiChuyen_Search?Code_G=f4cf3fe4b67244332eecec055742d449REG263ca1464c3480896c1b78a5eac5a6e971f0e3dad99908345f7439f8ffabdffc4_____DeviceName_____MAC_____LoginID_____2025-10-28&Search_txt=&Unit=2U01&Line=201S01&ALL=YES