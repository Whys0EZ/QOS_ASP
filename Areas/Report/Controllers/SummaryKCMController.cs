...
namespace QOS.Areas.Report.Controllers
{
    ...
    [Area("Report")]
    public class SummaryKCMController : Controller
    {
        private readonly ILogger<SummaryKCMController> _logger;
        ...
        {
            // return View();
            return RedirectToAction("RP_Summary_Defects_KCM", "SummaryKCM");
        }
        [TempData]
        public string? MessageStatus { get; set;}
        [HttpGet]
        public IActionResult RP_Summary_Defects_KCM(string? topDefected, string? typeCode, string? Unit, string? Line, string? Mo, string? styleCode, DateTime? dateFrom, DateTime? dateEnd)
        {
            _logger.LogInformation("=== RP_Summary_Defects_KCM GET Request ===");
            ...
            try
            {
                var model = new SummaryKCMViewModel
                {
                    ...
                    DefectStatKCMs = new Dictionary<string, DefectStatKCM>(),
                    Zone = GetZone()
            ...
            {
                _logger.LogError(ex, "Error in RP_Summary_Defects_KCM GET");
                TempData["ErrorMessage"] = $"Có lỗi xảy ra: {ex.Message}";
                return View(new SummaryKCMViewModel { Unit_List = GetUnitList() });
            }
        }
        ...
        private void LoadReportData(SummaryKCMViewModel model)
        {
            try
            {
                ...
                var sql = @"EXEC RP_TongHopLoiChuyenMay_MO @Date_From, @Date_To, @Unit, @Line, @MO, @StyleCode, @Defected_Type, @Top_Defected, @Search";
                ...
                var defectSummary = new Dictionary<string, DefectStatKCM>();
                ...
                while (reader.Read())
                {
                    ...
                    if (!string.IsNullOrEmpty(defectKey))
                    {
                        ...
                        {
                            defectSummary[defectKey] = new DefectStatKCM
                            {
                                Name = defectKey,
                                Count = faultQty,
                ...
                {
                    model.DefectStatKCMs = defectSummary
                ...
                _logger.LogInformation($"Statistics calculated - Total: {model.TotalDefects}, Defect Types: {model.DefectStatKCMs.Count}");
            ...
            catch (Exception ex)
            {
                ...
                model.DefectStatKCMs = new Dictionary<string, DefectStatKCM>();
        ...
        public IActionResult ExportToExcel( string? topDefected, string? typeCode, string? Unit, string? Line, string? Mo, string? styleCode, DateTime? dateFrom, DateTime? dateEnd, string faultCodes)
        {
            ...
            using var cmd = new SqlCommand("RP_TongHopLoiChuyenMay_DownloadExcelDetail_Line", conn);
            ...
            return File(stream.ToArray(),
                ...
                $"Report_InLine_{Unit}_{DateTime.Now:yyyyMMddHHmmss}.xlsx");
        }
