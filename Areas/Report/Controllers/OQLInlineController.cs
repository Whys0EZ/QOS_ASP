...
namespace QOS.Areas.Report.Controllers
{
    [Authorize]
    [Area("Report")]
    public class OQLInLineController : Controller
    {
        private readonly ILogger<OQLInLineController> _logger;
        ...
        public OQLInLineController(ILogger<OQLInLineController> logger, IWebHostEnvironment environment, IConfiguration configuration, AppDbContext context)
        {
            _logger = logger ;
            _env = environment ;
        ...
        {
            return RedirectToAction("RP_OQLInLine");
        }
        
        public ActionResult RP_OQLInLine(string unit = "1U01", int month = 0, int year = 0)
        {
            if (month == 0) month = DateTime.Now.Month;
            if (year == 0) year = DateTime.Now.Year;

            var model = new OQLInLineViewModel
            {
                SelectedUnit = unit,
                SelectedMonth = month,
                SelectedYear = year
            ...
            {
                var lineData = new OQLInLineData
                {
                    LineName = group.Key,
        ...
        public IActionResult DownloadReport(string unit, int month, int year)
        {
            ...
            using (var package = new OfficeOpenXml.ExcelPackage())
            {
                ...
                return File(stream, 
                        ...
                        $"OQL_Report_Inline_{unit}_{month}_{year}.xlsx");
            }
            // */
        ...
        public List<OQL_InLine> GetOQLData(string unit, int month, int year)
        {

            var data = new List<OQL_InLine>();
        ...
        public OQL_InLine? GetDayDetail(string unit, string line, DateTime date)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                connection.Open();
