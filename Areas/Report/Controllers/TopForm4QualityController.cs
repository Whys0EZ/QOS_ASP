...
namespace QOS.Areas.Report.Controllers
{
    ...
    [Area("Report")]
    public class TopForm4QualityController : Controller
    {
        private readonly ILogger<TopForm4QualityController> _logger;
        ...
        public TopForm4QualityController(ILogger<TopForm4QualityController> logger, IWebHostEnvironment env, IConfiguration configuration, AppDbContext context, IOptions<AppSettings> appSettings)
        {
            _logger = logger;
            _env = env;
        ...
        {
            // return View();
            return RedirectToAction("TopForm4Quality", "TopForm4Quality");
        }
        [TempData]
        public string? MessageStatus { get; set;}
        [HttpGet]
        public IActionResult TopForm4Quality(string? Unit, string? Line, DateTime? dateFrom, DateTime? dateEnd)
        {
            _logger.LogInformation("=== TopForm4Quality GET Request ===" + _appSettings.FactoryName );
        ...
        private void LoadReportData(TopForm4QualityViewModel model)
        {
            ...
            try
            {
                ...
                using (var command = new SqlCommand("RP_BCCLM_Report_Top5", connection))
                {
                    ...
                    
                    {
                        while(reader.Read())
                        {
                            ...
                            rowData["Style"] = reader["Style"]?.ToString() ?? "";
                            ...
                            rowData["Operation_Name_EN"] = reader["Operation_Name_EN"]?.ToString() ?? "";
        ...
        public IActionResult ExportExcel(string? Unit,string? Line, DateTime? dateFrom, DateTime? dateEnd)
        {
            ...
            var templatePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "Report", "RP_Top_Defect_Inline.xlsx");
            
            if (!System.IO.File.Exists(templatePath))
            {
                MessageStatus = "Không tìm thấy file mẫu báo cáo.";
                return RedirectToAction("TopForm4Quality");
            }
            ...
            using (var package = new ExcelPackage(new FileInfo(templatePath)))
            {
                ...
                using var cmd = new SqlCommand("RP_BCCLM_Report_Top5", conn);
                ...
                using (var reader = cmd.ExecuteReader())
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        worksheet.Cells[row, 2].Value = reader["Style"].ToString();
                        worksheet.Cells[row, 3].Value = reader["Operation_Code"].ToString();
                        worksheet.Cells[row, 4].Value = reader["Operation"].ToString();
                        worksheet.Cells[row, 5].Value = reader["Qty"].ToString();
                        worksheet.Cells[row, 6].Value = reader["Total_Fault_Qty"].ToString();
                        // worksheet.Cells[row, 7].Value = reader["defect_rate"].ToString();
                        ...
                        worksheet.Cells[row, 7].Value = OQL +"%" ;

                        // _logger.LogInformation("defectName : " + defectName);
                        worksheet.Cells[row, 8].Value = defectName;
                        row ++;
