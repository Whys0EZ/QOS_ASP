...
namespace QOS.Areas.Report.Controllers
{
    ...
    [Area("Report")]
    public class Form2BCCPIController : Controller
    {
        private readonly ILogger<Form2BCCPIController> _logger;
        ...
        {
            // return View();
            return RedirectToAction("RP_Form2", "Form2BCCPI");
        }
        ...
        public IActionResult RP_Form2(string? Unit, DateTime? dateFrom, DateTime? dateEnd)
        {
            ...
            var history = conn.Query<Form2_BCCPI>(sql, new { Unit, unitString, dateF = model.DateFrom, dateT = model.DateEnd }).ToList();
        ...
        public IActionResult DetailForm2(int id)
        {
            using var conn = new SqlConnection(_configuration.GetConnectionString("DefaultConnection"));
            string sql = @"
                SELECT t1.*, t4.FullName 
            ...
            var detail = conn.QueryFirstOrDefault<Form2_BCCPI>(sql, new { ID = id });
            ...
            return PartialView("_tableRP_Form2", detail);
        }
        public IActionResult DeleteReport(string reportId, int audit_Time)
        {
            try
            {
                ...
                using (var cmd = new SqlCommand("Delet_BC_Form2_BCCPI", conn))
                {
        ...
        public IActionResult ExportExcel(string? Unit, DateTime? dateFrom, DateTime? dateEnd)
        {
            ...
            ;
            // Console.WriteLine("SQL: " + sql + " Unit: " + Unit);
            var history = conn.Query<Form2_BCCPI>(sql, new { Unit, dateF = model.DateFrom, dateT = model.DateEnd }).ToList();
