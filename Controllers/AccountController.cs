...
namespace QOS.Controllers
{
    public class AccountController : Controller
    {
        ...
        public IActionResult Auth()
        {
            ...
            vm.Register.FactoryOptions = _context
                .Factory_List.Select(f => new SelectListItem
                {
                    Value = f.FactoryID,
                    Text = f.FactoryID,
                })
            ...
            vm.Register.TeamOptions = _context
                .Team_List.Select(t => new SelectListItem { Value = t.TeamID, Text = t.TeamName })
                .ToList();
        ...
        public async Task<IActionResult> Auth(AuthViewModel vm, string actionType)
        {
            ...
            if (actionType == "login")
            {
                // Xử lý Login
                if (
                    string.IsNullOrWhiteSpace(vm.Login.Username)
                    || string.IsNullOrWhiteSpace(vm.Login.Password)
                )
                {
                ...
                var user = _context
                    .Users.FromSqlRaw(
                        "EXEC Exec_Login_by_UserName @p0, @p1",
                        vm.Login.Username,
                        hash
                    )
                    .AsEnumerable()
                ...
                // Check detect moblie
                bool isMobile =
                    HttpContext.Items["IsMobile"] != null && (bool)HttpContext.Items["IsMobile"];
            ...
            else if (actionType == "register")
            {
                // Xử lý Register
                if (
                    string.IsNullOrWhiteSpace(vm.Register.FullName)
                    || string.IsNullOrWhiteSpace(vm.Register.Username)
                    || string.IsNullOrWhiteSpace(vm.Register.Pass)
                )
                {
                ...
                if (_context.Users.Any(u => u.Username == vm.Register.Username))
                {
                    ...
                    ModelState.AddModelError(
                        "Register.Username",
                        _sharedLocalizer["UsernameExist"]
                    );
                    ViewBag.ActiveForm = "register";
        ...
        {
            if (reg == null)
                reg = new UserEditViewModel();
            reg.FactoryOptions = _context
                .Factory_List.Select(f => new SelectListItem
                {
                    Value = f.FactoryID,
                    Text = f.FactoryID,
        ...
        )
        {
            ...
            try
            {
                ...
                if (AvatarFile != null && AvatarFile.Length > 0)
                {
                    ...
                    using (
                        var image = await SixLabors.ImageSharp.Image.LoadAsync(
